## Plan: Twilio Monthly Communication Numbers

Treat Twilio as a different product line from 5sim, not just another OTP provider. 5sim remains one-time, platform-specific activation for receiving verification codes only. Twilio becomes persistent monthly communication numbers that can receive OTPs and general SMS across multiple platforms and conversations during the subscription period. Implement Twilio with live inventory from Twilio APIs and short cache (no inventory DB persistence), USD to NGN conversion using existing exchange rate pipeline, Twilio-specific markup rules, reserve-first-then-debit purchase flow, and optional auto-renew with usage tracking so pricing/profit remains accurate and real-time.
...
- Included scope: Twilio monthly communication number rental, renewal lifecycle, send/receive SMS, provider-aware pricing and wallet flow.
- Product boundary rule: Twilio subscriptions must use separate entities/endpoints from 5sim activations (no overloading one-time activation semantics).
